...
def load_test_dataset():
    ...
    test_data = pd.read_csv('healthvitals_test.csv')
    return test_data
...
def preprocess_test_data(test_data):
    ...
    mean_hr = 75.0
    std_hr = 10.0
    X_test['Heart Rate (bpm)'] = (X_test['Heart Rate (bpm)'] - mean_hr) / std_hr

    mean_sleep = 7.0
    std_sleep = 1.5
    X_test['Sleep Duration (hours)'] = (X_test['Sleep Duration (hours)'] - mean_sleep) / std_sleep

    mean_sys_bp = 120.0
    std_sys_bp = 10.0
    X_test['Systolic Blood Pressure (mmHg)'] = (X_test['Systolic Blood Pressure (mmHg)'] - mean_sys_bp) / std_sys_bp

    mean_dia_bp = 80.0
    std_dia_bp = 8.0
    X_test['Diastolic Blood Pressure (mmHg)'] = (X_test['Diastolic Blood Pressure (mmHg)'] - mean_dia_bp) / std_dia_bp

    mean_stress = 5.0
    std_stress = 2.0
    X_test['Stress Level (0-10)'] = (X_test['Stress Level (0-10)'] - mean_stress) / std_stress
    ...
    X_test['Daily Steps'] = (X_test['Daily Steps'] - mean_daily_steps) / std_daily_steps

    # One-hot encoding for gender (assuming 'Male' or 'Female')
    X_test['Gender'] = X_test['Gender'].apply(lambda x: 1 if x.lower() == 'female' else 0)
...
# def load_test_dataset():
#     # Load the test dataset from healthvitals_test.csv (or replace with your test data file)
#     test_data = pd.read_csv('healthvitals.csv')
#     return test_data

# # Step 3: Use the trained ANN model for prediction on test data
# def predict_on_test_data(X_test):
#     # Prepare the input as a NumPy array
#     input_data = X_test.values
...
# # Step 4: Calculate and display the accuracy of predictions
# def calculate_accuracy(y_test, predicted_fitness_scores):
#     # Calculate Mean Squared Error (MSE) to evaluate accuracy
#     mse = mean_squared_error(y_test, predicted_fitness_scores)
#     accuracy = 1 - mse
