...
#         float)  # Convert gender back to integer
...
#     # Preprocess the input data
#     input_data = pd.DataFrame(input_data, columns=['Heart Rate', 'Sleep Duration', 'Systolic', 'Diastolic', 'Stress Level', 'Age', 'Daily Steps', 'Gender'])
    
#     # Preprocess categorical columns (like 'Gender') using one-hot encoding
#     categorical_cols = ['Gender']
#     ct = ColumnTransformer(
#         transformers=[('encoder', OneHotEncoder(), categorical_cols)], remainder='passthrough')
#     input_data['Gender'] = input_data['Gender'].map({'Male': 0, 'Female': 1})  # Map gender to numerical values
#     input_data = ct.transform(input_data)
...
# def predict_fitness_score(input_data):
#     # Load the pre-trained model
#     model = load_model(r'C:\Users\subra\Dropbox\My PC (LAPTOP-778OGN6L)\Desktop\techne\techne_ANN.h5')
...
#     # Load the dataset to fit the transformer and scaler
#     df = pd.read_csv('health_vitals_data.csv')
    
#     # Map gender to numerical values
#     input_data['Gender'] = input_data['Gender'].map({'Male': 0, 'Female': 1})
    
#     # Fit the transformer on the entire dataset (training data)
#     ct.fit(df)
    
#     # Normalize the data using the same scaler used during training
#     scaler = MinMaxScaler()
#     scaler.fit(ct.transform(df))
...
def predict_fitness_score(input_data):
    ...
    input_data = pd.DataFrame(input_data, columns=[
                              'Heart Rate', 'Sleep Duration', 'Systolic', 'Diastolic', 'Stress Level', 'Age', 'Daily Steps', 'Gender'])
